...
function mainController($scope, $http, $rootScope, $location){
            ...
            client.on('header', function(message){
                if(message === '0')
                    ...
                    **********************************************/ 
                    ...
                    search =  "<input type='text' id='boxSearch' class='boxSearch' placeholder='O que procura ?'>";
                    search += "<input type='submit' id='submitSearch' class='submitSearch' disabled value='ok'>";                    
                    search += "<div class='subOptionsSearch' id='subOptionsSearch' style='display: none;'>Campos";
                    search +=   "<ul>";
                        $.each(message[1], function(key,value){
                            search +=   "<li id='options_"+value.ID+"'> <a href='javascript:;'>"+value.Anuncios+"</a> </li>";
                        });
                    search +=   "</ul>";
                    search +=   "<select id='distancia'>";
                    search +=       "<option value='100000'>Qualquer lugar</option>";
                    search +=       "<option value='50000'>50 km</option>";
                    search +=       "<option value='25000'>25 km</option>";
                    search +=       "<option value='10000'>10 km</option>";
                    search +=       "<option value='5000'>5 km</option>";
                    search +=       "<option value='1000'>1 km</option>";
                    search +=       "<option value='500'>500 m</option>";
                    search +=   "</select>";
                    search += "</div>";
                    ...
                    montaSearch = $.tmpl("SearchTemplate", itensSearch);
                    ...
                    if($('.headerBase').length < 1){
                        ...
                        $("#contentSearch").append(montaSearch);
                     ...
                     * 
                    **********************************************/
                    $(".boxSearch").click(function () { $(".subOptionsSearch").show(); });

                    $(document).click(function (e) {
                        if (!$(e.target).hasClass("boxSearch") && $(e.target).parents(".subOptionsSearch").length === 0) 
                        {
                            $(".subOptionsSearch").hide();
                        }
                    });

                    $('.subOptionsSearch ul li').click(function(){
                    ...
                    let textInput,arrayOptionInput,distanceInput,pag;

                    $('#submitSearch').click(function(){
                        let resultInputSearch = $('#boxSearch').val();
                        let valInputActive    = $('.subOptionsSearch li.active a');
                        let selectDistance    = $('#distancia option:selected').val();
                        let arryInputActive = []; 

                        let parameterSearch = window.location.pathname;
                        url = parameterSearch.substring(parameterSearch.lastIndexOf('/')+1); 

                        $('.subOptionsSearch li.active a').each(function(){
                            arryInputActive.push($(this).text());
                        });

                        $.cookie('textInput', resultInputSearch);
                        $.cookie('arrayOptionInput', arryInputActive);
                        $.cookie('distanceInput', selectDistance);
                        $.cookie('pag', url);
                        
                        $('#boxSearch').val('');
                        $('.subOptionsSearch li').removeClass('active');
                        $('#distancia').val('100000');
                        $('#submitSearch').prop('disabled', true);
                        
                        $scope.$apply($location.path('/busca/'+0));
                    });                    
                    /**********************************************
                     *
                     * VISITANTES
                    ...
                    **********************************************/ 
                    ...
                    $.each(message[2], function(key,value){
                        $('#contentVisitante').html(value.Nome);
                        //search +=   "<li id='options_"+value.Nome+"'> <a href='javascript:;'>"+value.Anuncios+"</a> </li>";
                    });
                    /**********************************************
                     *
                     * FINAL VISITANTES
    ...
    **********************************************/
    ...
    client.on('verificaChat', function(message){
        if(message[0][0].Socket_From == $.jStorage.get("key")){
            let contentChat = $('#container_'+message[0][0].UsernameTo);
            if(contentChat.length < 1){
                openChat(message[0][0].UsernameTo, message[0][0].UsernameFrom);
            }
            
        };
    });
    client.on('recebeChat', function(message){
        if(message[0][0].Socket_From == $.jStorage.get("key")){
             $('#contentChat_'+message[0][0].UsernameFrom).append(`
            <div class="from-message" style="background: blue;">
                <span>`+message[0][0].Nome_to+`</span>
                <span>`+message[0][0].UsernameFrom+`</span>
                <span>`+message[0][0].Mensagem+`</span>
            </div>
            `); 
        }; 
        if(message[0][0].Socket_To == $.jStorage.get("key")){
            $('#contentChat_'+message[0][0].UsernameTo).append(`
            <div class="from-message" style="background: red;">
                <span>`+message[0][0].Nome_to+`</span>
                <span>`+message[0][0].UsernameFrom+`</span>
                <span>`+message[0][0].Mensagem+`</span>
            </div>
...
function closeChat(val){
    $('#container_'+val).remove(); 
}
function escondeChat(val){
...
function openChat(user_to, user_from){
        ...
        let boxChat = '';
            ...
            boxChat += '  <button onclick="escondeChat('+user_from+')">-</button>';
            boxChat += '  <button onclick="closeChat('+user_from+')">x</button>';
            boxChat += '  <div id="nome_'+user_from+'"></div>';
            boxChat += '  <img id="img_'+user_from+'" src="" width="50" />';
            boxChat += '  <p><div id="contentChat_'+user_from+'">';
            let recebeChat = function(array_to, array_from){
                $.each(array_from, function(key,value)
                {
                    $('#contentChat_'+value.UsernameTo).append(`
                    <div class="from-message" style="background: red;">
                        <span>`+value.Nome_to+`</span>
                        <span>`+value.UsernameFrom+`</span>
                        <span>`+value.Mensagem+`</span>
                    </div>
                    `);
                    $('#contentChat_'+value.UsernameFrom).append(`
                    <div class="from-message" style="background: blue;">
                        <span>`+value.Nome_to+`</span>
                        <span>`+value.UsernameFrom+`</span>
                        <span>`+value.Mensagem+`</span>
        ...
        /* VERIFICA SE EXISTE CONTENT CHAT, MOSTRA NO CORPO SO SITE E MOSTRA CONTEÚDO DE CONVERSA */
        if(windowsChat.length < 1){
            $('body').append(boxChat);
            
            client.emit('getInfoChat', user_from, function(message1){
                console.log(message1[0].IDUsername)
                if(message1[0].IDUsername != null){
                    
                    $("#img_"+user_from).attr('src',message1[0].Imagem);
                    $("#nome_"+user_from).append(message1[0].Nome);
                }                
            });   
            chat.emit('chat', user_to, user_from, function(message1, eventName){
                recebeChat(eventName, message1);
            });                                          
        };
        /* DIGITA E ENVIA CONVERSA PARA SERVIDOR */
        $('#keypress_'+user_from).keypress(function(e) {
            var currentString = $(this).val();
            var re= /<\S[^><]*>/g;  

            if(e.which == 13 && !e.shiftKey) {
              
                var key = $('#keypress_'+user_from).val();
   
                if(key != ''){
                    client.emit('chat', { message: currentString, to: user_to, from: user_from }, function(eventName, message){});
                }

                $('#keypress_'+user_from).val('');
            }            
        });
};
