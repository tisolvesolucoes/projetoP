        ...
        client.on('editar', function (message) {
            try {
                if (message != '') {
                    ...
                    estilosQueCurto += "<label> Estilos musicais que curto";
                    ...
                    fotoPerfi += '<input type="submit" id="btnFoto" disabled value="Upload Imagem" name="btnFoto">';
                    ...
                    musicaPerfil += '<input type="hidden" id="pasta1" value="' + message[0][0].PastasUsuario + '" />';
                    musicaPerfil += '<input type="file" name="myFileMusic" id="myFileMusic" accept=".mp3"/>';
                    musicaPerfil += '<input type="submit" id="btnMusica" value="Upload Musica" name="btnMusica">';
                    ...
                    //VIDEO
                    videoPerfil = '<form id="formVideos" name="formVideos" onsubmit="return false">';
                    videoPerfil += '<input type="text" name="myFileTitle" id="myFileTitle" placeholder="Titulo do video" />';
                    videoPerfil += '<input type="text" name="myFileVideo" id="myFileVideo" placeholder="Link do Youtube" />';
                    videoPerfil += '<input type="submit" id="btnVideo" value="Upload Video" name="submit">';
                    ...
                    anuncioPerfil += '<input type="text" name="tituloAnuncio" id="tituloAnuncio" value="" placeholder="Digite o titulo do anúncio" />';
                    anuncioPerfil += '<textarea name="textoAnuncio" id="textoAnuncio" value="" placeholder="Digite o texto do anúncio"></textarea>';
                    ...
                    for (var x = 0; x < message[12].length; x++) {
                        ...
                        anuncioPerfilList = `<div id="anuncio_` + message[12][x].ID + `" style="display: block !important">
                                    ...
                                    <input type="text" id="tituloAnuncio" value="`+ message[12][x].TitiloAnuncio + `" placeholder="Digite o titulo do Anúncio" />                                   
                                    ...
                                    <input type="text" id="Criacao" value="`+ message[12][x].DTCriacao + `" placeholder="Data criação do Anúncio" />
                ...
                **********************************************/
                ...
                client.on('retornoFile', function (message, eventName) {
                    setTimeout(function () {
                        if (message === '1') {
                            alert(message);
                            $('#status').html('');
                            ...
                            $('#myFile').val('');
                            $('#btnFoto').prop('disabled', true);
                            $('#' + eventName + ' #imagePerfil').attr('src', message);
                        }
                    }, 1500);
                });
                ...
                **********************************************/
                 ...
                 * 
                **********************************************/
                $('#formVideos').submit(function (event) {
...
acoes.metodos = {
    adiciona: function (valor, tipo, inputs) {
        ...
        resultado = function (uniq, tipos) {
            ...
            $('#formTELEFONES input[type=radio]').change(function () {
                var testdssd = $(this).parent();
                //console.log(testdssd.context.id);
                acoes.metodos.validaAtualizaTELEFONES();
            });
            $('#formEMAILS input[type=radio]').change(function () {
                var testdssd = $(this).parent();
                acoes.metodos.validaAtualizaEMAILS();
            });
        }
    },
        ...
        $.each($('#formTELEFONES input'), function () {
            $("#formTELEFONES").validate({
                submitHandler: function (form) {
                    ...
                    console.log(tokens, $.jStorage.get("key"), checked);
    ...
    validaFormAnuncio: function () {
        let validator = $("#formAnuncio").validate({
            ...
            messages: {
                titulo: {
                    required: "Preencha com Titulo do Anúncio"
                },
                textoAnuncio: {
                    required: "Preencha Texto do Anúncio"
                },
                tipoAnuncio: {
                    required: "Preencha Texto do Anúncio"
                },
                estados: {
                    required: "Preencha Estado do Anúncio"
                },
                cidades: {
                    required: "Preencha Cidade do Anúncio"
                },
                regiaoAnuncio: {
                    required: "Preencha região do Anúncio"
                },
                myFileAnuncio: {
                    required: "Insira Imagem do Anúncio"
                },
            },
            submitHandler: function (form) {
                $('#tituloAnuncio').val('');
