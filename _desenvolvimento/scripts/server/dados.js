    ...
    conn.app_pertube.query("CALL ATUALIZA_FRM_GERAL(\n        \"" + this.nome + "\",\n        \"" + this.sobrenome + "\",\n        \"" + this.username + "\",  \n        \"" + this.senha + "\",\n        \"" + this.data + "\",\n        \"" + this.sexo + "\",\n        \"" + this.sobre + "\",\n        \"" + this.id + "\")", function (err, result, _fields) {
        try {
            if (result[0][0].error === 0) {
                ...
                callback('success', 'Concluido com sucesso! :D');
            }
        }
        catch (error) {
            console.log(error);
...
var visitante = function (valor, array) {
    console.log(array.userLogado, array.userVisitado);
    conn.app_pertube.query("CALL VISITANTE(\"" + array.userLogado + "\",\"" + array.userVisitado + "\")", function (_err, result, _fields) {
        console.log(result);
    });
};
...
var busca = function (array, callback) {
    var texto = array.texto;
    var arrayInputs = array.array;
    var distancia = array.distancia;
    var page = array.pag;
    conn.app_pertube.query("\n        SELECT \n            IDUsuarios\n        FROM \n            auth\n        WHERE \n            IDSocket = \"" + array.userlogado + '"', function (_err, result2, _fields) {
        console.log(texto, arrayInputs, distancia, page, result2[0].IDUsuarios);
        conn.app_pertube.query('CALL BUSCA("' + texto + '", "' + arrayInputs + '", "' + distancia + '", "' + page + '", "' + result2[0].IDUsuarios + '")', function (_err, result, _fields) {
            callback(result);
        });
    });
};
...
var interacao = function (valor, tipo, to, from, typeNotif, parameter1, parameter2) {
    ...
    conn.app_pertube.query("\n        \n        select ID from usuarios where USERNAME = \"" + this.to_interacao + "\"\n        union all\n        select ID from usuarios where USERNAME = \"" + this.from_interacao + "\"", function (_err, result1, _fields) {
        console.log(tipo, typeNotif, result1[0].ID, result1[1].ID);
        try {
            conn.app_pertube.query('CALL ' + tipo + '("' + typeNotif + '","' + result1[0].ID + '","' + result1[1].ID + '")', function (_err, result, _fields) {
                switch (tipo) {
                    ...
                    case 'INSERTCHAT':
                        try {
                            console.log(result);
                            valor.emit('verificaChat', result);
                            valor.emit('recebeChat', result);
                        }
                        catch (error) {
                            console.log(error);
                        }
